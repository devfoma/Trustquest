...
export function ChainSelect({ chainName, chains = [], onChange = () => {} }) {
  ...
  const cache = useMemo(
    () =>
      chains.reduce((cache, chain) => {
        cache[chain.chain_name] = chain;
        return cache;
      }, {}),
    [chains]
  );
  ...
  return (
    <ThemeProvider>
      <Box display="flex" justifyContent="center" alignItems="center" className={themeClass}>
        <Combobox
          selectedKey={value}
          inputValue={input}
          onInputChange={(input) => {
          ...
          styleProps={{ width: { mobile: "100%", mdMobile: "350px" } }}
        >
          {options.map((option) => (
